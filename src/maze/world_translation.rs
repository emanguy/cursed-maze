...
pub fn create_walls_for_maze<'pillar>(maze: &Maze, pillars: &'pillar Vec<Vec<Pillar>>) -> Vec<Wall<'pillar, 'pillar>> {
    let mut walls = Vec::new();
    // First, create the walls at the edge of the maze. They will never be removed.

    // Top and bottom rows
    for col in 0..maze.cols() {
        walls.push(Wall::from_pillars(&pillars[0][col as usize], &pillars[0][(col + 1) as usize]));
        walls.push(Wall::from_pillars(&pillars[maze.rows() as usize][col as usize], &pillars[maze.rows() as usize][(col + 1) as usize]));
    }

    // Left and right sides
    for row in 0..maze.rows() {
        walls.push(Wall::from_pillars(&pillars[row as usize][0], &pillars[(row + 1) as usize][0]));
        walls.push(Wall::from_pillars(&pillars[row as usize][maze.cols() as usize], &pillars[(row + 1) as usize][maze.cols() as usize]));
    }
